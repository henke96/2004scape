...
const CoreOps: CommandHandlers = {
    ...
    [ScriptOpcode.PUSH_VARP]: state => {
        const primary = (state.intOperand & 0x10000) === 0;
        ...
        if (varpType.type === ScriptVarType.STRING) {
            state.pushString(activePlayer.getVar(varpType.id) as string);
        } else {
            state.pushInt(activePlayer.getVar(varpType.id) as number);
        }
    },
    ...
    [ScriptOpcode.POP_VARP]: state => {
        ...
        const varpType: VarPlayerType = check(state.intOperand & 0xffff, VarPlayerValid);
        if (varpType.protect && activePlayer.protectedAccessScript !== state) {
            throw new Error(`%${varpType.debugname} requires protected access`);
        }

        if (varpType.type === ScriptVarType.STRING) {
            activePlayer.setVar(varpType.id, state.popString());
        } else {
            activePlayer.setVar(varpType.id, state.popInt());
        }
    },
    ...
    [ScriptOpcode.PUSH_VARN]: state => {
        ...
        if (varnType.type === ScriptVarType.STRING) {
            state.pushString(activeNpc.getVar(varnType.id) as string);
        } else {
            state.pushInt(activeNpc.getVar(varnType.id) as number);
        }
    },
    ...
    [ScriptOpcode.POP_VARN]: state => {
        ...
        const varnType: VarNpcType = check(state.intOperand & 0xffff, VarNpcValid);
        if (varnType.type === ScriptVarType.STRING) {
            activeNpc.setVar(varnType.id, state.popString());
        } else {
            activeNpc.setVar(varnType.id, state.popInt());
        }
    },
