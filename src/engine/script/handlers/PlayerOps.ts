...
const PlayerOps: CommandHandlers = {
    ...
    [ScriptOpcode.COORD]: state => {
        const primary = state.intOperand === 0;
        const activePlayer = state.activePlayer(primary);
        state.pushInt(CoordGrid.packCoord(activePlayer.level, activePlayer.x, activePlayer.z));
    },
    ...
    [ScriptOpcode.P_APRANGE]: protectedAccessHandler(state => {
        ...
        activePlayer.apRange = check(state.popInt(), NumberNotNull);
        activePlayer.apRangeCalled = true;
    }),
    ...
    [ScriptOpcode.P_ARRIVEDELAY]: protectedAccessHandler(state => {
        ...
        if (activePlayer.lastMovement < World.currentTick) {
            return;
        }

        activePlayer.suspendedScript = state;
        activePlayer.delayedUntil = World.currentTick + 1;
    ...
    [ScriptOpcode.P_COUNTDIALOG]: protectedAccessHandler(state => {
        ...
        activePlayer.write(new PCountDialog());
        activePlayer.suspendedScript = state;
    ...
    [ScriptOpcode.P_DELAY]: protectedAccessHandler(state => {
        ...
        activePlayer.delayedUntil = World.currentTick + 1 + check(state.popInt(), NumberNotNull);
        activePlayer.suspendedScript = state;
        state.execution = ScriptState.DELAYED;
        state.corruptProtectedAccess(!primary);
    }),
    ...
    [ScriptOpcode.P_WALK]: protectedAccessHandler(state => {
        ...
        const primary = state.intOperand === 0;
        const activePlayer = state.activePlayer(primary);
        activePlayer.queueWaypoints(findPath(activePlayer.level, activePlayer.x, activePlayer.z, coord.x, coord.z));
    }),
    ...
    [ScriptOpcode.STAT_ADD]: state => {
        ...
        const current = activePlayer.levels[stat];
        const added = current + (constant + (current * percent) / 100);
        activePlayer.levels[stat] = Math.min(added, 255);
        if (stat === 3 && activePlayer.levels[3] >= activePlayer.baseLevels[3]) {
            activePlayer.heroPoints.clear();
        }
        if (added !== current) {
            activePlayer.changeStat(stat);
        }
    },
    ...
    [ScriptOpcode.STAT_SUB]: state => {
        ...
        activePlayer.levels[stat] = Math.max(subbed, 0);
        if (subbed !== current) {
            activePlayer.changeStat(stat);
        }
    },
    ...
    [ScriptOpcode.STAT_HEAL]: state => {
        ...
        const base = activePlayer.baseLevels[stat];
        const current = activePlayer.levels[stat];
        const healed = current + (constant + (current * percent) / 100);
        activePlayer.levels[stat] = Math.max(Math.min(healed, base), current);
    ...
    [ScriptOpcode.P_PREVENTLOGOUT]: protectedAccessHandler(state => {
        ...
        activePlayer.preventLogoutMessage = check(state.popString(), StringNotNull);
        activePlayer.preventLogoutUntil = World.currentTick + check(state.popInt(), NumberNotNull);
    }),
    ...
    [ScriptOpcode.P_EXACTMOVE]: protectedAccessHandler(state => {
        ...
        activePlayer.unsetMapFlag();
        activePlayer.exactMove(startPos.x, startPos.z, endPos.x, endPos.z, startCycle, endCycle, direction);
    }),
    ...
    [ScriptOpcode.BUSY]: state => {
        ...
        state.pushInt(!activePlayer.canAccess() || activePlayer.loggingOut ? 1 : 0);
    },
    ...
    [ScriptOpcode.BUSY2]: state => {
        ...
        state.pushInt(activePlayer.hasInteraction() || activePlayer.hasWaypoints() ? 1 : 0);
    },
    ...
    [ScriptOpcode.GETQUEUE]: state => {
        ...
        let count: number = 0;
        for (let request = activePlayer.queue.head(); request !== null; request = activePlayer.queue.next()) {
            if (request.script.id === scriptId) {
                count++;
            }
        }
        for (let request= activePlayer.weakQueue.head(); request !== null; request = activePlayer.weakQueue.next()) {
            if (request.script.id === scriptId) {
                count++;
            }
        }
    ...
    [ScriptOpcode.BAS_RUNNING]: state => {
        ...
        const seq = state.popInt();
        if (seq === -1) {
            activePlayer.basRunning = -1;
            return;
        }
        activePlayer.basRunning = check(seq, SeqTypeValid).id;
    },
    ...
    [ScriptOpcode.HINT_PLAYER]: state => {
        ...
        activePlayer.hintPlayer(target.pid);
    },
    ...
    [ScriptOpcode.P_OPPLAYER]: protectedAccessHandler(state => {
        ...
        activePlayer.stopAction();
        activePlayer.setInteraction(Interaction.SCRIPT, target, ServerTriggerType.APPLAYER1 + type);
    }),
    ...
    [ScriptOpcode.HEALENERGY]: state => {
        ...
        const primary = state.intOperand === 0;
        const activePlayer = state.activePlayer(primary);
        activePlayer.runenergy = Math.min(Math.max(activePlayer.runenergy + amount, 0), 10000);
    },
    ...
    [ScriptOpcode.SETIDKIT]: state => {
        ...
        if (activePlayer.gender === 1) {
            slot -= 7;
        }
        activePlayer.body[slot] = idkType.id;
        ...
        if (colorSlot !== -1) {
            activePlayer.colors[colorSlot] = color;
        }
    },
    ...
    [ScriptOpcode.SETGENDER]: state => {
        ...
        for (let i = 0; i < 7; i++) {
            if(gender === 1) {
                activePlayer.body[i] = Player.MALE_FEMALE_MAP.get(activePlayer.body[i]) ?? -1;
            } else {
                if(i == 1) {
                    activePlayer.body[i] = 14;
                    continue;
                }
                activePlayer.body[i] = Player.FEMALE_MALE_MAP.get(activePlayer.body[i]) ?? -1;
            }
        }
        activePlayer.gender = gender;
    },
    ...
    [ScriptOpcode.P_OPPLAYERT]: protectedAccessHandler(state => {
        ...
        activePlayer.stopAction();
        activePlayer.setInteraction(Interaction.SCRIPT, target, ServerTriggerType.APPLAYERT, {type: -1, com: spellId});
    }),
    ...
    [ScriptOpcode.P_RUN]: protectedAccessHandler(state => {
        ...
        activePlayer.run = state.popInt();
        // todo: better way to sync engine varp
        activePlayer.setVar(VarPlayerType.RUN, activePlayer.run);
    }),
};
