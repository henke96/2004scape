...
const ObjOps: CommandHandlers = {
    ...
    [ScriptOpcode.OBJ_ADD]: state => {
        ...
        if (!objType.stackable) {
            for (let i = 0; i < count; i++) {
                ...
                World.addObj(obj, activePlayer.hash64, duration);
    ...
    [ScriptOpcode.OBJ_DEL]: state => {
        const primary = state.intOperand === 0;
        const activeObj = state.activeObj(primary);

        const duration: number = ObjType.get(activeObj.type).respawnrate;
        World.removeObj(activeObj, duration);
    },
    ...
    [ScriptOpcode.OBJ_COORD]: state => {
        const primary = state.intOperand === 0;
        const activeObj = state.activeObj(primary);
        state.pushInt(CoordGrid.packCoord(activeObj.level, activeObj.x, activeObj.z));
    },
