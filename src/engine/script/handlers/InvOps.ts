...
const InvOps: CommandHandlers = {
    ...
    [ScriptOpcode.INV_ADD]: state => {
        ...
        if (invType.protect && invType.scope !== InvType.SCOPE_SHARED && activePlayer.protectedAccessScript !== state) {
        ...
        const overflow = count - activePlayer.invAdd(invType.id, objType.id, count, false);
        if (overflow > 0) {
            if (!objType.stackable || overflow === 1) {
                for (let i = 0; i < overflow; i++) {
                    const obj = new Obj(activePlayer.level, activePlayer.x, activePlayer.z, EntityLifeCycle.DESPAWN, objType.id, 1);
                    World.addObj(obj, activePlayer.hash64, 200);
                }
            } else {
                const obj = new Obj(activePlayer.level, activePlayer.x, activePlayer.z, EntityLifeCycle.DESPAWN, objType.id, overflow);
                World.addObj(obj, activePlayer.hash64, 200);
            }
        }
    },
    ...
    [ScriptOpcode.INV_CHANGESLOT]: state => {
        ...
        const fromInv = activePlayer.getInventory(inv);
        ...
        for (let slot = 0; slot < fromInv.capacity; slot++) {
            ...
            if(obj.id === findObj.id) {
                activePlayer.invSet(invType.id, replaceObj.id, replaceCount, slot);
                return;
            }
        }
    },
    ...
    [ScriptOpcode.INV_CLEAR]: state => {
        ...
        activePlayer.invClear(invType.id);
    },
    ...
    [ScriptOpcode.INV_DEL]: state => {
        ...
        activePlayer.invDel(invType.id, objType.id, count);
    },
    ...
    [ScriptOpcode.INV_DELSLOT]: state => {
        ...
        const obj = activePlayer.invGetSlot(invType.id, slot);
        if (!obj) {
            return;
        }

        activePlayer.invDelSlot(invType.id, slot);
    },
    ...
    [ScriptOpcode.INV_MOVEFROMSLOT]: state => {
        ...
        if (fromInvType.protect && fromInvType.scope !== InvType.SCOPE_SHARED && activePlayer.protectedAccessScript !== state) {
            throw new Error(`$from_inv requires protected access: ${fromInvType.debugname}`);
        }

        if (toInvType.protect && toInvType.scope !== InvType.SCOPE_SHARED && activePlayer.protectedAccessScript !== state) {
            throw new Error(`$to_inv requires protected access: ${toInvType.debugname}`);
        }

        const { overflow, fromObj } = activePlayer.invMoveFromSlot(fromInvType.id, toInvType.id, fromSlot);
        if (overflow > 0) {
            const objType: ObjType = ObjType.get(fromObj);
            if (!objType.stackable || overflow === 1) {
                for (let i = 0; i < overflow; i++) {
                    const obj = new Obj(activePlayer.level, activePlayer.x, activePlayer.z, EntityLifeCycle.DESPAWN, fromObj, 1);
                    World.addObj(obj, activePlayer.hash64, 200);
                }
            } else {
                const obj = new Obj(activePlayer.level, activePlayer.x, activePlayer.z, EntityLifeCycle.DESPAWN, fromObj, overflow);
                World.addObj(obj, activePlayer.hash64, 200);
            }
        }
    },
    ...
    [ScriptOpcode.INV_MOVETOSLOT]: state => {
        ...
        activePlayer.invMoveToSlot(fromInvType.id, toInvType.id, fromSlot, toSlot);
    },
    ...
    [ScriptOpcode.INV_MOVEITEM]: state => {
        ...
        const completed = activePlayer.invDel(fromInvType.id, objType.id, count);
        if (completed == 0) {
            return;
        }

        const overflow = count - activePlayer.invAdd(toInvType.id, objType.id, completed, false);
        if (overflow > 0) {
            if (!objType.stackable || overflow === 1) {
                for (let i = 0; i < overflow; i++) {
                    const obj = new Obj(activePlayer.level, activePlayer.x, activePlayer.z, EntityLifeCycle.DESPAWN, objType.id, 1);
    ...
    [ScriptOpcode.INV_MOVEITEM_CERT]: state => {
        ...
        const overflow = count - activePlayer.invAdd(toInvType.id, finalObj, completed, false);
        if (overflow > 0) {
            // should be a stackable cert already!
            const obj = new Obj(activePlayer.level, activePlayer.x, activePlayer.z, EntityLifeCycle.DESPAWN, finalObj, overflow);
            World.addObj(obj, activePlayer.hash64, 200);
        }
    ...
    [ScriptOpcode.INV_MOVEITEM_UNCERT]: state => {
        ...
        if (objType.certtemplate >= 0 && objType.certlink >= 0) {
            activePlayer.invAdd(toInvType.id, objType.certlink, completed);
        } else {
    ...
    [ScriptOpcode.INV_SETSLOT]: state => {
        ...
        activePlayer.invSet(invType.id, objType.id, count, slot);
    },
    ...
    [ScriptOpcode.INV_TRANSMIT]: state => {
        ...
        activePlayer.invListenOnCom(invType.id, com, activePlayer.uid);
    },
    ...
    [ScriptOpcode.INV_DROPALL]: state => {
        ...
        const inventory: Inventory | null = activePlayer.getInventory(invType.id);
        if (!inventory) {
            return;
        }
        ...
        for (let slot: number = 0; slot < inventory.capacity; slot++) {
            ...
            const objType: ObjType = ObjType.get(obj.id);
            if (invType.scope === InvType.SCOPE_PERM) {
                activePlayer.addWealthLog(-(obj.count * objType.cost), `Dropped ${objType.debugname} x${obj.count}`);
            }
