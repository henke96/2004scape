...
const NpcOps: CommandHandlers = {
    ...
    [ScriptOpcode.NPC_COORD]: state => {
        const primary = state.intOperand === 0;
        const activeNpc = state.activeNpc(primary);
        state.pushInt(CoordGrid.packCoord(activeNpc.level, activeNpc.x, activeNpc.z));
    },

    [ScriptOpcode.NPC_DEL]: state => {
        const primary = state.intOperand === 0;
        const activeNpc = state.activeNpc(primary);
        World.removeNpc(activeNpc, check(activeNpc.type, NpcTypeValid).respawnrate);
    },
    ...
    [ScriptOpcode.NPC_DELAY]: state => {
        ...
        activeNpc.cancelSuspendedScript();

        activeNpc.delayedUntil = World.currentTick + 1 + check(state.popInt(), NumberNotNull);
        activeNpc.suspendedScript = state;
    ...
    [ScriptOpcode.NPC_RANGE]: state => {
        ...
        const primary = state.intOperand === 0;
        const activeNpc = state.activeNpc(primary);
        if (coord.level !== activeNpc.level) {
            state.pushInt(-1);
        } else {
            state.pushInt(
                CoordGrid.distanceTo(activeNpc, {
                    x: coord.x,
                    z: coord.z,
                    width: 1,
                    length: 1
    ...
    [ScriptOpcode.NPC_SETHUNTMODE]: state => {
        ...
        const activeNpc = state.activeNpc(primary);
        if (huntTypeId === -1) {
            activeNpc.huntMode = -1;
        } else {
            activeNpc.huntMode = check(huntTypeId, HuntTypeValid).id;
        }
    },
    ...
    [ScriptOpcode.NPC_SETMODE]: state => {
        ...
        if (target) {
            if (target instanceof Npc || target instanceof Obj || target instanceof Loc) {
                activeNpc.setInteraction(Interaction.SCRIPT, target, mode, {type: target.type, com: -1});
            } else {
                activeNpc.setInteraction(Interaction.SCRIPT, target, mode);
            }
        } else {
            activeNpc.noMode();
        }
    ...
    [ScriptOpcode.NPC_STATHEAL]: state => {
        ...
        const base = activeNpc.baseLevels[stat];
        const current = activeNpc.levels[stat];
        const healed = current + (constant + (current * percent) / 100);
        activeNpc.levels[stat] = Math.min(healed, base);

        // reset hero points if hp current == base
        if (stat === 0 && activeNpc.levels[stat] === activeNpc.baseLevels[stat]) {
            activeNpc.heroPoints.clear();
        }
    },
    ...
    [ScriptOpcode.NPC_WALKTRIGGER]: state => {
        ...
        activeNpc.walktrigger = queueId - 1;
        activeNpc.walktriggerArg = arg;
    },
    ...
    [ScriptOpcode.NPC_STATADD]: state => {
        ...
        activeNpc.levels[stat] = Math.min(added, 255);
    ...
    [ScriptOpcode.NPC_STATSUB]: state => {
        ...
        activeNpc.levels[stat] = Math.max(subbed, 0);
    },
    ...
    [ScriptOpcode.NPC_ARRIVEDELAY]: state => {
        ...
        if (activeNpc.lastMovement < World.currentTick - 1) {
            return;
        }
        // If npc moved 1 tick ago, delay for 1 tick. If npc moved this tick, delay for 2 ticks
        if (activeNpc.lastMovement === World.currentTick - 1) {
            activeNpc.delayedUntil = World.currentTick + 1;
        } else {
            activeNpc.delayedUntil = World.currentTick + 2;
        }
        activeNpc.suspendedScript = state;
        state.execution = ScriptState.DELAYED;
        state.corruptProtectedAccess(true);
